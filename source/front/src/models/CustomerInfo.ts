...
export class CustomerInfo extends Stateful {
  ...
  constructor(app: App) {
    ...
    this.validation = new Validation(new Map<CustomerKeys, PropertyValidator>([
      ...
      ['homePhoneNumber', new PropertyValidator(this.app, 'homePhoneNumber', /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/)],
      ['mobilePhoneNumber', new PropertyValidator(this.app, 'mobilePhoneNumber', /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/)],
